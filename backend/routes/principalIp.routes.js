...
const { normalizePrincipalHost } = require('../utils/principalDiscovery');
...
async function testPrincipalConnection(ip, timeoutMs = 3000) {
  ...
  try {
    ...
    if (data.principalSessionOpen !== true) {
      throw new Error("Le poste répond, mais aucune session de caisse principale n'y est ouverte.");
    }
    return data;
  } finally {
...
router.post('/', (req, res) => {
  const ip = normalizePrincipalHost(req.body?.ip);
...
router.post('/test', async (req, res) => {
  const ip = normalizePrincipalHost(req.body?.ip || getConfig().ip);
