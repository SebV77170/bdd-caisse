    ...
    db.transaction(() => {
      logs.forEach(log => {
        ...
        if (type === 'ticketdecaisse') {
          ...
          if (operation === 'INSERT') {
            db.prepare(`INSERT OR IGNORE INTO bilan (date, timestamp, nombre_vente, poids, prix_total, prix_total_espece, prix_total_cheque, prix_total_carte, prix_total_virement) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
              .run(data.date, data.timestamp, safe(data.nombre_vente), safe(data.poids),
                   safe(data.prix_total), safe(data.prix_total_espece), safe(data.prix_total_cheque),
                   safe(data.prix_total_carte), safe(data.prix_total_virement));
          } else if (operation === 'UPDATE') {
            db.prepare(`UPDATE bilan SET timestamp = ?, nombre_vente = nombre_vente + ?, poids = poids + ?, prix_total = prix_total + ?, prix_total_espece = prix_total_espece + ?, prix_total_cheque = prix_total_cheque + ?, prix_total_carte = prix_total_carte + ?, prix_total_virement = prix_total_virement + ? WHERE date = ?`)
              .run(data.timestamp, safe(data.nombre_vente), safe(data.poids), safe(data.prix_total),
                   safe(data.prix_total_espece), safe(data.prix_total_cheque), safe(data.prix_total_carte),
                   safe(data.prix_total_virement), data.date);
          }
